...
class RetroAchievementsApi @Inject constructor(
    ...
    private val hashCacheMutex = Mutex()
    private val hashCache = mutableMapOf<Int, Map<String, String>>()

    /**
     * The RA game id whose hash list contains [hash] on [consoleId], or null. RA identifies games
     * solely by content hash, so this is the only automatic RA match. Caches per console.
     */
    suspend fun gameIdForHash(consoleId: Int, hash: String): String? {
        val map = hashCacheMutex.withLock {
